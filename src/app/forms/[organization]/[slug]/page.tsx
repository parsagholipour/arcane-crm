...
export default async function MarketingFormPage({ params }: { params: Params }) {
  ...
  const resolvedOrganization = await prisma.organization.findUnique({ where: { slug: organization }, select: { id: true, name: true } });
  if (!resolvedOrganization) notFound();
  const page = await prisma.marketingLandingPage.findFirst({ where: { organizationId: resolvedOrganization.id, slug, status: "Published" }, select: { name: true, slug: true, headline: true, description: true, submitLabel: true, successMessage: true, fields: true } });
  if (!page) notFound();
