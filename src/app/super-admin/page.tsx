...
const primaryClass =
  "inline-flex h-9 items-center justify-center rounded bg-brand-600 px-4 text-sm font-semibold text-white hover:bg-brand-700";
...
export default async function SuperAdminPage({
  ...
  for (const delivery of invitationDeliveries) {
    if (delivery.sourceId && !latestInvitationDeliveries.has(delivery.sourceId))
      latestInvitationDeliveries.set(delivery.sourceId, delivery);
  }
    ...
    <main className="mx-auto max-w-[1500px] space-y-6 p-5">
      <div>
        ...
        <p className="text-sm text-[#706e6b]">
          Global identity operations affect every organization a user belongs to.
        </p>
      </div>
      {params.message && (
      ...
      {params.error && (
        <div className="rounded border border-red-300 bg-red-50 p-3 text-sm text-red-800">{params.error}</div>
      )}
      ...
      <section className="space-y-3">
        ...
        {organizations.map((organization) => {
          const activeAdmins = organization.memberships.filter(
            (row) => row.role === "ADMIN" && row.status === "ACTIVE" && row.user.status === "ACTIVE"
          ).length;
          return (
            <article key={organization.id} className="rounded border border-[#d8dde6] bg-white p-4 shadow-sm">
              ...
              <div className="mt-3 overflow-x-auto">
                <table className="w-full text-left text-sm">
                  <thead className="bg-[#f3f3f3] text-xs uppercase text-[#706e6b]">
                    <tr>
                  ...
                  <tbody>
                    {organization.memberships.map((membership) => {
                      ...
                      const invitationFailed = ["Bounced", "Dropped", "Spam Report", "Unsubscribed"].includes(
                        invitationStatus
                      );
                      ...
                      return (
                        <tr key={membership.id} className="border-t align-top">
                          <td className="p-2">
                            ...
                            <div className="text-xs text-[#706e6b]">{membership.user.email ?? "Legacy identity"}</div>
                          </td>
                          <td className="p-2">
                            <form
                              action={updateMembershipAction.bind(null, membership.id)}
                              className="flex flex-wrap gap-2"
                            >
                              <input type="hidden" name="organizationId" value={organization.id} />
                              <select className={inputClass} name="role" defaultValue={membership.role}>
                          ...
                          <td className="p-2">
                            <div className={invitationFailed ? "font-semibold text-red-700" : "font-semibold"}>
                              {invitationStatus}
                            </div>
                            {invitationAt && (
                              <div className="text-xs text-[#706e6b]">{invitationAt.toLocaleString()}</div>
                            )}
                            {delivery?.lastReason && (
                              <div className="max-w-xs text-xs text-red-700">{delivery.lastReason}</div>
                            )}
                          </td>
                          <td className="p-2">
                            <div className="flex flex-wrap gap-3">
                              <form action={resendOrganizationInvitationAction.bind(null, membership.id)}>
                                ...
                                <button
                                  ...
                                  disabled={membership.status !== "ACTIVE" || membership.user.status !== "ACTIVE"}
                                >
                                  Resend invitation
                                </button>
                              </form>
      ...
      <section className="rounded border border-[#d8dde6] bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <form className="flex gap-2">
            <input className={inputClass} name="q" defaultValue={query} placeholder="Search name or email" />
          ...
          {users.map((user) => (
            <article key={user.id} className="rounded border p-3">
              <div className="grid gap-3 xl:grid-cols-[1fr_auto]">
                <form
                  action={updateGlobalUserAction.bind(null, user.id)}
                ...
                >
                  ...
                  <input
                    ...
                    disabled={!user.keycloakSub}
                  />
                  <button className={buttonClass} disabled={!user.keycloakSub}>
                    Save identity
                  </button>
                </form>
                <div className="flex flex-wrap gap-2">
                  <form action={setGlobalUserStatusAction.bind(null, user.id)}>
                    <input type="hidden" name="status" value={user.status === "ACTIVE" ? "SUSPENDED" : "ACTIVE"} />
                    <button className={buttonClass} disabled={!user.keycloakSub}>
                      {user.status === "ACTIVE" ? "Suspend globally" : "Reactivate"}
                    </button>
                  </form>
                  <form action={sendUserActionsAction.bind(null, user.id)}>
                  ...
                  <form action={revokeUserSessionsAction.bind(null, user.id)}>
                    <button className={buttonClass}>Revoke sessions</button>
                  </form>
                </div>
              </div>
              <div className="mt-2 text-xs text-[#706e6b]">
                {user.status} · {user.appSessions.length} active app sessions · Last login{" "}
                {user.lastLoginAt?.toLocaleString() ?? "Never"} · Setup email{" "}
                {user.setupEmailSentAt?.toLocaleString() ?? "Not recorded"}
              </div>
              <div className="mt-2 flex flex-wrap gap-2">
                {user.memberships.map((membership) => (
                  <span key={membership.id} className="rounded bg-[#eef4ff] px-2 py-1 text-xs">
                    {membership.organization.name}: {membership.role}/{membership.status}
                  </span>
                ))}
              </div>
            </article>
