...
import {
  ...
  optionalLandingPageText,
  requireLandingPage,
  validateLandingPageReferences
  ...
  try {
    ...
    if (existing.status !== "Draft")
      return NextResponse.json(
        { error: "Only Draft landing pages can be edited. Restore an archived page to Draft first." },
        { status: 409 }
      );
    const payload = await request.json();
    if (payload.status !== undefined)
      return NextResponse.json({ error: "Use a lifecycle action to change landing-page status." }, { status: 400 });
    ...
    const headline =
      payload.headline === undefined ? existing.headline : landingPageText(payload.headline, "Headline", "headline");
    const description =
      payload.description === undefined
        ? existing.description
        : optionalLandingPageText(payload.description, "description", 2000);
    const submitLabel =
      payload.submitLabel === undefined
        ? existing.submitLabel
        : landingPageText(payload.submitLabel, "Submit Label", "submitLabel", 60);
    const successMessage =
      payload.successMessage === undefined
        ? existing.successMessage
        : landingPageText(payload.successMessage, "Success Message", "successMessage", 500);
    const fields = payload.fields === undefined ? existing.fields : landingPageFields(payload.fields);
    const campaignId =
      payload.campaignId === undefined ? existing.campaignId : String(payload.campaignId ?? "").trim() || null;
    ...
    const page = await prisma.marketingLandingPage.update({
      where: { id },
      data: { name, slug, headline, description, submitLabel, successMessage, fields, campaignId, ownerId },
...
export async function DELETE(_request: NextRequest, context: { params: Params }) {
  try {
    ...
    if (existing.status !== "Draft" || existing._count.submissions > 0)
      return NextResponse.json(
        { error: "Only Draft landing pages without submissions can be deleted." },
        { status: 409 }
      );
