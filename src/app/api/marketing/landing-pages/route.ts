...
export async function GET() {
  try {
    ...
    const landingPages = await prisma.marketingLandingPage.findMany({
...
export async function POST(request: NextRequest) {
  try {
    ...
    const successMessage = landingPageText(
      payload.successMessage || "Thanks. Your information was received.",
      "Success Message",
      "successMessage",
      500
    );
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const notification = await createMarketingPageNotification(tx, {
        ...
        body: `${page.name} was created as a Draft.`
      });
