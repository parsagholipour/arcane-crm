...
export async function POST(request: NextRequest, context: { params: Params }) {
  try {
    ...
    if (action === "attendance") {
      ...
      if (!["Invited", "Accepted", "Declined", "Attended", "No Show"].includes(attendance)) return NextResponse.json({ error: "Choose a valid attendance status." }, { status: 400 });
    ...
    const transition =
      existing.status === "Scheduled" && action === "start" ? { status: "In Progress", startedAt: new Date(), endedAt: null } :
      existing.status === "Scheduled" && action === "cancel" ? { status: "Cancelled", startedAt: null, endedAt: new Date() } :
      existing.status === "In Progress" && action === "complete" ? { status: "Completed", startedAt: existing.startedAt ?? new Date(), endedAt: new Date() } : null;
    if (!transition) return NextResponse.json({ error: `Cannot ${action || "perform that action"} while the video call is ${existing.status}.` }, { status: 409 });
    const videoCall = await prisma.videoCall.update({ where: { id }, data: transition, include: videoCallInclude });
    const notification = await createVideoCallNotification(authContext.organizationId, authContext.userId, `Video call ${videoCall.status.toLowerCase()}`, `${videoCall.name} is now ${videoCall.status}.`, id);
