...
import { buildVideoParticipants, requireVideoCall, requireVideoProvider, validateVideoDates, validateVideoReferences, validateWebUrl, videoCallErrorResponse, videoCallInclude } from "@/lib/video-calls";
...
export async function GET(_request: NextRequest, context: { params: Params }) {
  try {
    ...
    return NextResponse.json({ videoCall: JSON.parse(JSON.stringify(videoCall)), capabilities: { emailInvitations: emailDeliveryConfigured() } });
  } catch (error) {
...
export async function PATCH(request: NextRequest, context: { params: Params }) {
  try {
    ...
    if (payload.status !== undefined) return NextResponse.json({ error: "Use a lifecycle action to change video-call status." }, { status: 400 });
    if (["Cancelled", "Completed"].includes(existing.status)) {
      const allowed = new Set(["notes", "recordingUrl"]);
      if (Object.keys(payload).some((key) => !allowed.has(key))) return NextResponse.json({ error: `${existing.status} video calls only allow notes and recording updates.` }, { status: 409 });
    }
    ...
    const dates = payload.scheduledStartAt !== undefined || payload.scheduledEndAt !== undefined
      ? validateVideoDates(payload.scheduledStartAt ?? existing.scheduledStartAt, payload.scheduledEndAt ?? existing.scheduledEndAt)
      : null;
    const organizerId = await validateVideoReferences(authContext.organizationId, authContext.userId, { ...payload, organizerId: payload.organizerId ?? existing.organizerId });
    const participants = payload.participants === undefined ? null : await buildVideoParticipants(authContext.organizationId, payload.participants);
    const videoCall = await prisma.$transaction(async (tx) => {
      if (participants) await tx.videoCallParticipant.deleteMany({ where: { videoCallId: id, organizationId: authContext.organizationId } });
      return tx.videoCall.update({
        ...
        data: {
          ...
          opportunityId: payload.opportunityId === undefined ? undefined : String(payload.opportunityId ?? "").trim() || null,
          organizerId,
          recordingUrl: payload.recordingUrl === undefined ? undefined : validateWebUrl(payload.recordingUrl, "Recording URL"),
...
export async function DELETE(_request: NextRequest, context: { params: Params }) {
  try {
    ...
    const { id } = await context.params;
    const existing = await requireVideoCall(authContext.organizationId, id);
    if (existing.status !== "Scheduled" || existing.startedAt) return NextResponse.json({ error: "Only video calls that have not started can be deleted." }, { status: 409 });
