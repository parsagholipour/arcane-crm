...
export async function POST(request: NextRequest, context: { params: Params }) {
  try {
    ...
    if (session.status === "Closed")
      return NextResponse.json({ error: "Closed messaging sessions cannot receive new messages." }, { status: 409 });
    ...
    if (!["Inbound", "Outbound", "System"].includes(direction))
      throw new MessagingValidationError("Choose a valid message direction.");
    const deliver = payload.deliver === true;
    let delivery: Awaited<ReturnType<typeof sendTrackedEmail>> | null = null;
    if (deliver) {
      if (direction !== "Outbound" || session.channel !== "Email")
        throw new MessagingValidationError("Provider delivery is only available for outbound Email sessions.");
      if (!emailDeliveryConfigured())
        throw new MessagingValidationError("Email delivery is not configured for this CRM.", 503);
      const address = String(
        payload.recipient ??
          session.contact?.email ??
          session.participants.find((participant) => isValidEmail(participant.address))?.address ??
          ""
      );
      if (!isValidEmail(address))
        throw new MessagingValidationError("Choose a participant with a valid email address.");
    ...
    const message = await prisma.$transaction(async (tx) => {
      const created = await tx.messagingMessage.create({
        data: {
          ...
          direction,
          senderName:
            String(payload.senderName ?? (direction === "Outbound" ? authContext.user.name : "")).trim() || null,
      ...
      await tx.messagingSession.update({
        where: { id },
        data: { lastMessageAt: now, status: direction === "Inbound" ? "Open" : undefined }
      });
      return created;
    });
    await attachTrackedDeliveries(delivery?.deliveryIds, {
      ...
      sourceType: "MessagingMessage",
      sourceId: message.id
    });
    const notification = await createMessagingNotification(
      authContext.organizationId,
      authContext.userId,
      delivery ? "Message accepted for email delivery" : "Message recorded",
      `${session.name}: ${body.slice(0, 120)}`,
      id
    );
    return NextResponse.json(
      { message: JSON.parse(JSON.stringify(message)), delivery, notifications: [notification] },
      { status: 201 }
    );
  } catch (error) {
    console.error(error);
