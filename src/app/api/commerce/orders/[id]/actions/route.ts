...
export async function POST(request: NextRequest, { params }: { params: Params }) {
  try { const context = await requireOrganizationContext(); const { id } = await params; const payload = await request.json(); const result = await transitionCommerceOrder(context.organizationId, context.userId, id, String(payload.action ?? ""), payload); return NextResponse.json(JSON.parse(JSON.stringify(result))); }
