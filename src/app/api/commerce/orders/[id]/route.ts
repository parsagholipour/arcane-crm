...
export async function GET(_request: NextRequest, { params }: { params: Params }) {
  try {
    ...
    return NextResponse.json({
      order: JSON.parse(JSON.stringify(await requireCommerceOrder(context.organizationId, id)))
    });
  } catch (error) {
    ...
    return domain
      ...
      : NextResponse.json({ error: "Unable to load order." }, { status: 500 });
  }
}
export async function PATCH(request: NextRequest, { params }: { params: Params }) {
  try {
    const context = await requireOrganizationContext();
    const { id } = await params;
    const order = await updateCommerceOrder(context.organizationId, context.userId, id, await request.json());
...
export async function DELETE(_request: NextRequest, { params }: { params: Params }) {
  try {
    const context = await requireOrganizationContext();
    const { id } = await params;
    await deleteCommerceOrder(context.organizationId, id);
    ...
    return domain
      ? NextResponse.json({ error: domain.error }, { status: domain.status })
      : NextResponse.json({ error: "Unable to delete order." }, { status: 500 });
  }
}
