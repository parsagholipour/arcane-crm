...
export async function GET(request: NextRequest) {
  try {
    ...
    const orders = await prisma.commerceOrder.findMany({
      where: { organizationId: context.organizationId, ...(status ? { status } : {}), ...(storeId ? { storeId } : {}) },
      include: commerceOrderInclude,
      orderBy: { orderDate: "desc" }
    });
    return NextResponse.json({ orders: JSON.parse(JSON.stringify(orders)) });
  } catch (error) {
    return authorizationErrorResponse(error) ?? NextResponse.json({ error: "Unable to load orders." }, { status: 500 });
  }
}
export async function POST(request: NextRequest) {
  try {
    const context = await requireOrganizationContext();
    const result = await createCommerceOrder(context.organizationId, context.userId, await request.json());
