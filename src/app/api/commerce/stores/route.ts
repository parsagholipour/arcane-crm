...
export async function GET() {
  try {
    ...
    const stores = await prisma.marketingStore.findMany({ where: { organizationId: context.organizationId }, include: commerceStoreInclude, orderBy: { updatedAt: "desc" } });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const domain = commerceErrorResponse(error); if (domain) return NextResponse.json({ error: domain.error, field: domain.field }, { status: domain.status });
    console.error(error); return NextResponse.json({ error: "Unable to create store." }, { status: 500 });
  }
}
