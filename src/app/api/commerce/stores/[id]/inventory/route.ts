...
export async function GET(_request: NextRequest, { params }: { params: Params }) {
  try {
    ...
    return NextResponse.json({ inventory: JSON.parse(JSON.stringify(inventory)) });
  } catch (error) {
    ...
    return domain
      ...
      : NextResponse.json({ error: "Unable to load inventory." }, { status: 500 });
  }
}
export async function POST(request: NextRequest, { params }: { params: Params }) {
  try {
    const context = await requireOrganizationContext();
    const { id } = await params;
    const inventoryItem = await upsertInventory(context.organizationId, id, await request.json());
    return NextResponse.json({ inventoryItem: JSON.parse(JSON.stringify(inventoryItem)) });
  } catch (error) {
    const auth = authorizationErrorResponse(error);
    if (auth) return auth;
    const domain = commerceErrorResponse(error);
    return domain
      ...
      : NextResponse.json({ error: "Unable to update inventory." }, { status: 500 });
  }
}
