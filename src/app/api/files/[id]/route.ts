  ...
  try {
    ...
    if (!stored.content)
      return NextResponse.json(
        { error: "File content is unavailable for this legacy metadata-only record." },
        { status: 410 }
      );
...
export async function DELETE(request: NextRequest, context: { params: Params }) {
  try {
    ...
    if (stored.attachment)
      await prisma.attachmentRecord.deleteMany({ where: { id, organizationId: authContext.organizationId } });
