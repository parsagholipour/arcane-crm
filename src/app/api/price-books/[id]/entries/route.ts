...
export async function GET(_request: NextRequest, { params }: { params: Params }) {
  try {
    ...
    const book = await prisma.priceBook.findFirst({ where: { id, organizationId: context.organizationId } });
    if (!book) return NextResponse.json({ error: "Price Book not found." }, { status: 404 });
    const entries = await prisma.priceBookEntry.findMany({
      where: { organizationId: context.organizationId, priceBookId: id },
      include: { product: true, priceBook: true }
    });
    return NextResponse.json({ priceBook: book, entries: JSON.parse(JSON.stringify(entries)) });
  } catch (error) {
    return (
      authorizationErrorResponse(error) ??
      NextResponse.json({ error: "Unable to load Price Book entries." }, { status: 500 })
    );
  }
}
...
export async function POST(request: NextRequest, { params }: { params: Params }) {
  try {
    ...
    const listPrice = decimal(payload.listPrice);
    if (!listPrice || listPrice.lt(0))
      return NextResponse.json({ error: "List price must be zero or greater." }, { status: 400 });
    const duplicate = await prisma.priceBookEntry.findFirst({
      where: { organizationId: context.organizationId, priceBookId: id, productId, currency }
    });
    if (duplicate)
      return NextResponse.json(
        { error: "This Product and currency already have an entry in the Price Book." },
        { status: 409 }
      );
    ...
    return (
      authorizationErrorResponse(error) ??
      NextResponse.json({ error: "Unable to create Price Book entry." }, { status: 500 })
    );
  }
}
