...
export async function POST(request: NextRequest, context: { params: Params }) {
  try {
    ...
    if (campaign.status === "Archived")
      return NextResponse.json({ error: "Archived campaigns cannot receive members." }, { status: 409 });
    const payload = await request.json();
    const objectType = String(payload.objectType ?? "");
    const recordIds: string[] = Array.from(
      new Set<string>(
        Array.isArray(payload.recordIds) ? payload.recordIds.map((value: unknown) => String(value)).filter(Boolean) : []
      )
    );
    const status = String(payload.status ?? "Sent");
    if (!["Lead", "Contact"].includes(objectType))
      throw new CampaignValidationError("Campaign members must be Leads or Contacts.");
    ...
    const count =
      objectType === "Lead"
        ? await prisma.lead.count({ where: { organizationId: authContext.organizationId, id: { in: recordIds } } })
        : await prisma.contact.count({ where: { organizationId: authContext.organizationId, id: { in: recordIds } } });
    if (count !== recordIds.length)
      throw new AppAuthorizationError("One or more selected campaign members were not found.", 404);
    await prisma.$transaction(
      recordIds.map((recordId) =>
        prisma.campaignMember.upsert({
          ...
          update: {
            status,
            responded: ["Responded", "Attended", "Converted"].includes(status),
            firstRespondedAt: ["Responded", "Attended", "Converted"].includes(status) ? new Date() : undefined
          },
          create: {
            organizationId: authContext.organizationId,
            campaignId: id,
            ...
            status,
            responded: ["Responded", "Attended", "Converted"].includes(status),
            firstRespondedAt: ["Responded", "Attended", "Converted"].includes(status) ? new Date() : null
          }
        })
      )
    );
    ...
    const notification = await createCampaignNotification(
      ...
      `${recordIds.length} ${objectType.toLowerCase()} record${recordIds.length === 1 ? "" : "s"} added to ${refreshed.name}.`,
      id
    );
    ...
    return NextResponse.json({ error: "Unable to add campaign members." }, { status: 500 });
  }
}
