    ...
    async (tx) => {
      ...
      for (const lead of leads) {
        ...
        let account = options.existingAccountId
          ...
          : await tx.account.findFirst({ where: { organizationId, name: { equals: accountName, mode: "insensitive" } } });
        if (options.existingAccountId && !account) throw new WorkflowValidationError("The selected Account was not found.", 409);
        if (!account) {
        ...
        let contact = options.existingContactId
          ...
          : null;
        if (options.existingContactId && !contact) throw new WorkflowValidationError("The selected Contact was not found.", 409);
        if (contact) {
        ...
        if (options.createOpportunity) {
          if (options.existingOpportunityId) {
            opportunity = await tx.opportunity.findFirst({ where: { organizationId, id: options.existingOpportunityId } });
...
async function reparentLeadHistory(tx: PrismaTransaction, organizationId: string, leadId: string, contactId: string) {
  for (const [from, to] of [["Lead", "Contact"], ["Leads", "Contacts"]] as const) {
...
async function moveCampaignMembers(tx: PrismaTransaction, organizationId: string, leadId: string, contactId: string) {
  ...
  if (moveIds.length) {
    await tx.campaignMember.updateMany({ where: { id: { in: moveIds } }, data: { objectType: "Contact", recordId: contactId } });
  }
}
...
async function moveRecordLabels(tx: PrismaTransaction, organizationId: string, leadId: string, contactId: string) {
  ...
  if (moveIds.length) {
    await tx.recordLabel.updateMany({ where: { id: { in: moveIds } }, data: { objectType: "Contact", recordId: contactId } });
  }
}
