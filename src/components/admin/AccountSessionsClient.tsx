...
export function AccountSessionsClient() {
  ...
  async function load() {
    try {
      const payload = await apiRequest<{
        appSessions?: AppSessionRow[];
        keycloakSessions?: KeycloakSessionRow[];
      }>("/api/account/sessions", { cache: "no-store" });
  ...
  async function action(body: Record<string, string>) {
    try {
      await apiRequest<Record<string, unknown>>("/api/account/sessions", {
        method: "POST",
        body: jsonBody(body)
      });
      setMessage("Session access updated.");
      await load();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Unable to update sessions.");
    }
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <button
        ...
        onClick={() => void action({ action: "logout-others" })}
      >
        Log out other sessions
      </button>
      <SessionTable
        title="Application sessions"
        rows={appSessions.map((row) => ({
          id: row.id,
          current: row.current,
          primary: row.userAgent || "Unknown browser",
          secondary: `${row.ipAddress || "Unknown IP"} · Last seen ${new Date(row.lastSeenAt).toLocaleString()}`,
          source: "app"
        }))}
        onRevoke={action}
      />
      <SessionTable
        ...
        rows={keycloakSessions.map((row) => ({
          id: row.id,
          current: row.current,
          primary: Object.values(row.clients ?? {}).join(", ") || "Keycloak session",
          secondary: `${row.ipAddress || "Unknown IP"}${row.lastAccess ? ` · Last seen ${new Date(row.lastAccess).toLocaleString()}` : ""}`,
          source: "keycloak"
        }))}
        onRevoke={action}
      />
...
function SessionTable({
  ...
  title: string;
  rows: Array<{ id: string; current: boolean; primary: string; secondary: string; source: string }>;
  onRevoke: (body: Record<string, string>) => void;
}) {
  return (
      ...
      {rows.map((row) => (
        <div key={row.id} className="flex items-center gap-3 border-b p-3 last:border-b-0">
          <div className="min-w-0 flex-1">
            ...
            <div className="truncate text-xs text-[#706e6b]">{row.secondary}</div>
          </div>
          {!row.current && (
            <button
              className="rounded border border-red-300 px-3 py-1.5 text-sm font-semibold text-red-700"
              onClick={() => onRevoke({ action: "revoke", source: row.source, sessionId: row.id })}
            >
              Revoke
            </button>
          )}
