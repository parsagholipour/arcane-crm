...
const buttonClass =
  "inline-flex h-9 items-center justify-center rounded border border-brand-600 bg-white px-3 text-sm font-semibold text-brand-700 hover:bg-brand-50 disabled:opacity-50";
...
export function OrganizationUsersClient({ initialMemberships }: { initialMemberships: Membership[] }) {
  ...
  async function request(path: string, init: RequestInit) {
    ...
    setMessage(null);
    try {
      return await apiRequest<MembershipPayload>(path, init);
    } catch (error) {
  ...
  async function invite(formData: FormData) {
    ...
    const membership: Membership = {
      ...payload.membership,
      invitationDelivery: payload.invitationDelivery ?? null
    };
  ...
  async function update(membership: Membership, role: Membership["role"], status: Membership["status"]) {
    const payload = await request(`/api/organization/members/${membership.id}`, {
      method: "PATCH",
    ...
    setMemberships((current) =>
      current.map((row) =>
        row.id === membership.id ? { ...updatedMembership, invitationDelivery: row.invitationDelivery } : row
      )
    );
    ...
    <div className="space-y-5">
      ...
      <section className="rounded border border-[#d8dde6] bg-white p-4 shadow-sm">
        ...
        <p className="mt-2 text-xs text-[#706e6b]">
          Every membership receives an organization invitation. New identities also receive a separate Keycloak setup
          email.
        </p>
      </section>
      ...
      <section className="overflow-hidden rounded border border-[#d8dde6] bg-white shadow-sm">
        <table className="w-full text-left text-sm">
          <thead className="bg-[#f3f3f3] text-xs uppercase text-[#706e6b]">
            <tr>
              ...
              <th className="p-3">Organization access</th>
        ...
        {memberships.length === 0 && (
          <div className="p-8 text-center text-sm text-[#706e6b]">No users belong to this organization.</div>
        )}
      </section>
    </div>
  );
  ...
  return (
    <tr className="border-t align-top">
      <td className="p-3">
        ...
        <div className="text-xs text-[#706e6b]">{membership.user.email}</div>
        <div className="text-xs text-[#706e6b]">
          Last login: {membership.user.lastLoginAt ? new Date(membership.user.lastLoginAt).toLocaleString() : "Never"}
        </div>
      </td>
      ...
      <td className="p-3">
        <div className="flex flex-wrap gap-2">
          <select
            ...
            value={role}
            onChange={(event) => setRole(event.target.value as Membership["role"])}
          >
          ...
          <select
            className={inputClass}
            value={status}
            onChange={(event) => setStatus(event.target.value as Membership["status"])}
          >
      ...
      <td className="p-3">
        <InvitationStatus membership={membership} />
      </td>
      <td className="p-3">
        <div className="flex flex-wrap gap-2">
          <button className={buttonClass} disabled={busy} onClick={() => onUpdate(membership, role, status)}>
            Save
          </button>
          <button
            className={buttonClass}
            disabled={busy || membership.status !== "ACTIVE" || membership.user.status !== "ACTIVE"}
            onClick={() => onResend(membership)}
          >
            Resend invitation
          </button>
          <button
            className="h-9 rounded px-3 text-sm font-semibold text-red-700 hover:bg-red-50"
            disabled={busy}
