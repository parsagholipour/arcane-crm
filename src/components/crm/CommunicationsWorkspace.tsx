...
export function MessagingSessionEditorModal({ data, initial, onClose, onSaved, onToast }: { data: BootstrapData; initial?: RecordData; onClose: () => void; onSaved: (result: CommunicationsMutationResult) => void; onToast: (toast: Toast) => void }) {
  ...
  return (
    <DialogShell title={initial ? `Edit ${text(initial.name)}` : "New Messaging Session"} onClose={requestClose} wide footer={<><button className={secondaryButton} onClick={requestClose}>Cancel</button><AsyncButton className={primaryButton} onClick={() => save()} disabled={saving}>{saving ? "Saving…" : "Save"}</AsyncButton></>}>
...
function MessageModal({ session, capabilities, onClose, onSaved }: { session: RecordData; capabilities: { emailDelivery: boolean }; onClose: () => void; onSaved: (result: CommunicationsMutationResult) => void }) {
  ...
  return <DialogShell title="Record Message" onClose={onClose} footer={<><button className={secondaryButton} onClick={onClose}>Cancel</button><AsyncButton className={primaryButton} onClick={() => save()} disabled={saving}>{saving ? "Saving…" : canDeliver && deliver ? "Send Email & Record" : "Record Message"}</AsyncButton></>}>
