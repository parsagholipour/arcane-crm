...
export type InvoiceMutationResult = { invoice: RecordData; notifications?: RecordData[] };
          ...
          {confirmDiscard ? (
            ...
            <>
              <div className="flex items-center justify-between border-b border-[#d8dde6] px-5 py-4">
                <div><Dialog.Title className="text-xl font-semibold">{mode === "new" ? "New Invoice" : `Edit ${text(invoice?.invoiceNumber)}`}</Dialog.Title><Dialog.Description className="mt-0.5 text-sm text-[#706e6b]">Sales invoice - separate from application subscription billing</Dialog.Description></div>
...
export function InvoicePaymentModal({ invoice, onClose, onSaved, onToast }: { invoice: RecordData; onClose: () => void; onSaved: (result: InvoiceMutationResult) => void; onToast: (toast: InvoiceToast) => void }) {
  ...
  return <SimpleDialog title="Record Payment" description="Record an externally received payment. This does not process or transfer money." onClose={onClose} footer={<><InvoiceButton onClick={onClose}>Cancel</InvoiceButton><InvoiceButton tone="primary" disabled={saving} onClick={() => void submit()}>{saving ? "Recording..." : "Record Payment"}</InvoiceButton></>}>
...
function InvoiceSendModal({ invoice, initialRecipient, onClose, onSent, onToast }: {
  ...
  return <SimpleDialog title="Send Invoice" description="The invoice PDF will be attached. The invoice becomes Sent after SendGrid accepts the message." onClose={onClose} footer={<><InvoiceButton onClick={onClose}>Cancel</InvoiceButton><InvoiceButton tone="primary" disabled={working} onClick={() => void submit()}><Send size={14} /> {working ? "Sending..." : "Send Invoice"}</InvoiceButton></>}>
...
function InvoiceButton({ children, onClick, tone = "secondary", disabled = false }: { children: ReactNode; onClick: () => void; tone?: "primary" | "secondary" | "danger"; disabled?: boolean }) {
  return <button type="button" disabled={disabled} onClick={onClick} className={cn("inline-flex min-h-8 items-center justify-center gap-1.5 rounded border px-3 py-1.5 text-xs font-semibold transition disabled:cursor-not-allowed disabled:opacity-50", tone === "primary" ? "border-[#0176d3] bg-[#0176d3] text-white hover:bg-[#014486]" : tone === "danger" ? "border-[#ba0517] bg-white text-[#ba0517] hover:bg-[#fff1f1]" : "border-[#c9c9c9] bg-white text-[#181818] hover:bg-[#f3f3f3]")}>{children}</button>;
}
