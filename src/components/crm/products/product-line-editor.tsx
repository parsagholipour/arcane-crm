...
export function ProductLineEditor({
  ...
  async function save() {
    ...
    const collection = config.path(id(subject));
    try {
      const payload = await apiRequest<{ product: RecordData }>(line ? `${collection}/${id(line)}` : collection, {
      ...
      setError(saveError instanceof Error ? saveError.message : config.saveError);
    }
  }
  ...
  return (
    <BaseDialog
      open
      title={line ? `Edit ${text(linkedProduct?.name) || config.lineLabel}` : config.addDialogTitle}
      onClose={onClose}
      onEnterAction={save}
      footer={
        <>
