...
export function validateRecordPayload(object: CrmObject, payload: RecordData) {
  ...
  if (object === "Opportunity") {
    ...
    optionalChoice(
      ...
      FORECAST_CATEGORIES,
      "Choose a valid Forecast Category.",
      "forecastCategory"
    );
  ...
  if (object === "Event") {
    ...
    if (
      ...
      !parseRecurrenceRule(payload.recurrenceRule)
    ) {
      throw new RecordPayloadValidationError(
        "This repeat pattern isn't supported. Choose a daily, weekly, monthly, or yearly repeat.",
        ["recurrenceRule"]
      );
    }
    if (payload.reminderMinutes !== null && payload.reminderMinutes !== undefined) {
      ...
      if (!Number.isInteger(reminderMinutes) || reminderMinutes < 0 || reminderMinutes > MAX_EVENT_REMINDER_MINUTES) {
        throw new RecordPayloadValidationError(
          `Reminder must be a whole number of minutes from 0 through ${MAX_EVENT_REMINDER_MINUTES} (four weeks).`,
          ["reminderMinutes"]
        );
      }
    }
  ...
  if (object === "ListEmail")
    optionalDate(payload.scheduledAt, "Choose a valid scheduled date and time.", "scheduledAt");
...
function optionalNonNegativeInteger(value: unknown, label: string, field: string) {
  ...
  if (!Number.isInteger(number) || number < 0)
    throw new RecordPayloadValidationError(`${label} must be a non-negative whole number.`, [field]);
}
...
function validatePriceBookDates(payload: RecordData) {
  ...
  if (from && to && to < from)
    throw new RecordPayloadValidationError("Valid To cannot precede Valid From.", ["validTo"]);
}
...
function combinedDate(dateValue: unknown, timeValue: unknown, field: string) {
  ...
  const date = new Date(
    source.includes("T") ? source : `${source}T${String(timeValue ?? "00:00").slice(0, 5)}:00.000Z`
  );
  if (!Number.isFinite(date.getTime()))
    throw new RecordPayloadValidationError("Choose a valid date and time.", [field]);
  return date;
}
