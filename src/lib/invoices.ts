...
async function prepareAggregate(
  ...
  if (!account)
    throw new InvoiceDomainError("The selected Account was not found in this organization.", 404, "accountId");
  ...
  if (opportunityId) {
    ...
    if (!opportunity)
      throw new InvoiceDomainError(
        "The selected Opportunity was not found in this organization.",
        404,
        "opportunityId"
      );
    if (opportunity.accountId !== accountId)
      throw new InvoiceDomainError(
        "The selected Opportunity must belong to the invoice Account.",
        400,
        "opportunityId"
      );
  }
  ...
  if (!/^[A-Z]{3}$/.test(currency))
    throw new InvoiceDomainError("Currency must be a three-letter ISO code.", 400, "currency");
  ...
  const normalizedLines = rawLines.map((line, index) => {
    if (!line || typeof line !== "object" || Array.isArray(line))
      throw new InvoiceDomainError(`Line ${index + 1} is invalid.`, 400, `lineItems.${index}`);
    return line as RecordData;
  });

  const productIds = Array.from(
    new Set(normalizedLines.map((line) => optionalText(line.productId)).filter((id): id is string => Boolean(id)))
  );
  const products = productIds.length
    ? await tx.product.findMany({ where: { organizationId, id: { in: productIds } } })
    : [];
  if (products.length !== productIds.length)
    throw new InvoiceDomainError(
      "One or more selected Products were not found in this organization.",
      404,
      "lineItems.productId"
    );
  ...
  priceEntries.forEach((entry) => {
    if (entry.listPrice && !pricesByProductId.has(entry.productId))
      pricesByProductId.set(entry.productId, entry.listPrice);
  });
    ...
    calculated = calculateInvoiceTotals(
      normalizedLines.map((line, index) => {
        ...
        return toLineInput(
          ...
          productId ? pricesByProductId.get(productId) : undefined
        );
      })
    );
  } catch (error) {
  ...
  return {
    header: {
      ...
      billingStreet: optionalText(
        valueFrom(payload, "billingStreet", existing?.billingStreet ?? account.billingStreet)
      ),
      ...
      billingPostalCode: optionalText(
        valueFrom(payload, "billingPostalCode", existing?.billingPostalCode ?? account.billingPostalCode)
      ),
      billingCountry: optionalText(
        valueFrom(payload, "billingCountry", existing?.billingCountry ?? account.billingCountry)
      ),
...
async function allocateInvoiceNumber(tx: Transaction, organizationId: string) {
  ...
  if (!Number.isInteger(allocated) || allocated < 1)
    throw new InvoiceDomainError("Unable to allocate an invoice number.", 409);
  return `INV-${String(allocated).padStart(6, "0")}`;
}

async function createNotification(
  tx: Transaction,
  values: { organizationId: string; userId: string; title: string; body: string; invoiceId: string }
) {
...
export async function getInvoice(organizationId: string, userId: string, id: string) {
  ...
  await prisma.globalSearchRecent.upsert({
    ...
    create: {
      ...
      label: invoice.invoiceNumber,
      context: "Invoice",
      href: `/lightning/r/Invoice/${id}/view`,
      category: "Record"
    }
  });
  return invoice;
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (hasOwn(payload, "lineItems")) {
      ...
      if (calculated.lineItems.length)
        await tx.invoiceLineItem.createMany({ data: calculated.lineItems.map((line) => ({ invoiceId: id, ...line })) });
    }
...
export async function deleteDraftInvoice(organizationId: string, id: string) {
  return prisma.$transaction(async (tx) => {
    ...
    if (invoice.payments.length)
      throw new InvoiceDomainError("An invoice with payment history cannot be deleted.", 409);
    await tx.invoiceLineItem.deleteMany({ where: { invoiceId: id } });
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (action === "mark-sent") {
      ...
      if (!existing.lineItems.length)
        throw new InvoiceDomainError(
          "Add at least one valid line item before marking the invoice Sent.",
          400,
          "lineItems"
        );
      status = "Sent";
      ...
      if (!["Sent", "Partially Paid", "Overdue"].includes(existing.status))
        throw new InvoiceDomainError("Only an outstanding invoice can be voided.", 409);
      if (existing.payments.length || existing.amountPaid.gt(0))
        throw new InvoiceDomainError("Invoices with payments cannot be voided without a reversal mechanism.", 409);
      status = "Void";
...
export async function markInvoiceSentAfterDelivery(
  organizationId: string,
  userId: string,
  id: string,
...
export async function recordInvoicePayment(organizationId: string, userId: string, id: string, payload: RecordData) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (!["Sent", "Partially Paid", "Overdue"].includes(existing.status)) {
      throw new InvoiceDomainError(
        "Payments can only be recorded against Sent, Partially Paid, or Overdue invoices.",
        409
      );
    }
    const currentPaid = money(
      existing.payments.reduce((sum, payment) => sum.plus(payment.amount), new Prisma.Decimal(0))
    );
    const outstanding = money(existing.total.minus(currentPaid));
    if (amount.gt(outstanding))
      throw new InvoiceDomainError(
        `Payment cannot exceed the outstanding balance of ${outstanding.toFixed(2)} ${existing.currency}.`,
        400,
        "amount"
      );
