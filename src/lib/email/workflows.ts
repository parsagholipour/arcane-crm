...
export async function deliverListEmail(input: {
  ...
  const delivery = await sendTrackedEmail({
    ...
    text: body,
    scheduledAt
  }, { organizationId: input.organizationId, userId: input.userId, sourceType: "ListEmail", sourceId: input.sourceId }, dependencies);
  return { ...delivery, skipped: resolved.skipped };
}

export async function deliverCaseNotification(input: {
  organizationId: string;
  organizationName: string;
  userId: string;
  sourceId?: string;
  ...
  if (!isValidEmail(contact.email)) throw new EmailValidationError("The selected case contact does not have a valid email address.");
  ...
  return sendTrackedEmail({
    ...
    text: template.text
  }, { organizationId: input.organizationId, userId: input.userId, sourceType: "Case", sourceId: input.sourceId }, dependencies);
}
