...
export async function deliverInvoiceEmail(input: {
  ...
  const invoice = await prisma.invoice.findFirst({ where: { id: input.invoiceId, organizationId: input.organizationId }, include: invoiceInclude });
  ...
  if (!invoice.lineItems.length) throw new InvoiceDomainError("Add at least one valid line item before sending the invoice.", 400, "lineItems");
  ...
  }, { organizationId: input.organizationId, userId: input.userId, sourceType: "Invoice", sourceId: input.invoiceId }, dependencies);
  const result = await markInvoiceSentAfterDelivery(input.organizationId, input.userId, input.invoiceId, recipientEmail);
