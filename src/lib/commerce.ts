...
export const commerceOrderInclude = {
  ...
  contact: true,
  lines: {
    include: { product: true, priceBookEntry: { include: { priceBook: true } } },
    orderBy: [{ sortOrder: "asc" as const }, { createdAt: "asc" as const }]
  },
  promotions: { include: { promotion: true } },
  fulfillments: {
    include: { lines: { include: { orderLine: { include: { product: true } } } } },
...
function decimal(value: unknown, label: string, scale = 2) {
  try {
    const result = new Prisma.Decimal(
...
export function storeSlug(value: unknown) {
  const result = requiredText(value, "Store name")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-|-$/g, "")
    .slice(0, 60);
...
export async function createCommerceNotification(
  ...
  return tx.notification.create({
    data: {
      ...
      title: values.title,
      body: values.body,
      href: values.href ?? "/lightning/app/commerce",
      category: "Commerce"
    }
  });
}
...
export async function createCommerceStore(organizationId: string, userId: string, payload: RecordData) {
  ...
  if (!/^[A-Z]{3}$/.test(currency))
    throw new CommerceDomainError("Currency must be a three-letter ISO code.", 400, "currency");
  const priceBookId = text(payload.priceBookId);
  if (
    priceBookId &&
    !(await prisma.priceBook.findFirst({ where: { id: priceBookId, organizationId }, select: { id: true } }))
  )
    throw new CommerceDomainError("Price Book not found.", 404, "priceBookId");
  if (await prisma.marketingStore.findFirst({ where: { organizationId, slug }, select: { id: true } }))
    throw new CommerceDomainError("That store URL slug is already in use.", 409, "slug");
  return prisma.$transaction(async (tx) => {
    const store = await tx.marketingStore.create({
      data: {
        organizationId,
        name,
        slug,
    ...
    const notification = await createCommerceNotification(tx, {
      ...
      userId,
      title: "Store created",
      body: `${name} was created as a Draft.`
    });
...
export async function updateCommerceStore(organizationId: string, userId: string, id: string, payload: RecordData) {
  ...
  if (existing.status === "Archived")
    throw new CommerceDomainError("Archived stores cannot be edited. Restore the store first.", 409);
  ...
  const duplicate = await prisma.marketingStore.findFirst({
  ...
  if (
    ...
    throw new CommerceDomainError("Price Book not found.", 404, "priceBookId");
  const store = await prisma.marketingStore.update({
    where: { id },
    data: {
      name,
      slug,
      currency: payload.currency === undefined ? undefined : requiredText(payload.currency, "Currency").toUpperCase(),
      description: payload.description === undefined ? undefined : text(payload.description),
      priceBookId
    },
    include: commerceStoreInclude
  });
  await createCommerceNotification(prisma, {
    organizationId,
    userId,
    title: "Store updated",
    body: `${store.name} settings were updated.`
  });
  return store;
}
...
export async function transitionCommerceStore(organizationId: string, userId: string, id: string, action: string) {
  return prisma.$transaction(async (tx) => {
    ...
    if (normalized === "activate" && existing.status === "Draft") {
      if (!existing.priceBookId)
        throw new CommerceDomainError("Choose a Price Book before activating the store.", 400, "priceBookId");
      const activeEntries = await tx.priceBookEntry.count({
        where: { organizationId, priceBookId: existing.priceBookId, active: true, product: { active: true } }
      });
      if (!activeEntries)
        throw new CommerceDomainError(
          "Activate at least one Product and Price Book entry before activating the store.",
          400
        );
      status = "Active";
      ...
      throw new CommerceDomainError(
        `Cannot ${normalized || "perform that action"} while the store is ${existing.status}.`,
        409
      );
    const now = new Date();
    const store = await tx.marketingStore.update({
      where: { id },
      data: {
        status,
        launchedAt: status === "Active" ? (existing.launchedAt ?? now) : undefined,
        archivedAt: status === "Archived" ? now : status === "Draft" ? null : undefined
      },
      include: commerceStoreInclude
    });
    const notification = await createCommerceNotification(tx, {
...
async function allocateOrderNumber(tx: Transaction, organizationId: string) {
  ...
  if (!Number.isInteger(allocated) || allocated < 1)
    throw new CommerceDomainError("Unable to allocate an order number.", 409);
...
async function prepareOrder(
  ...
  payload: RecordData,
  existing?: CommerceOrderAggregate
) {
  ...
  if (store.status === "Archived")
    throw new CommerceDomainError("Archived stores cannot receive orders.", 409, "storeId");
  ...
  if (contactId) {
    ...
    if (contact.accountId !== accountId)
      throw new CommerceDomainError("The Contact must belong to the selected Account.", 400, "contactId");
  }
  ...
  if (currency !== store.currency)
    throw new CommerceDomainError(`Order currency must match the store currency (${store.currency}).`, 400, "currency");
  ...
  const lines = sourceLines.map((line, index) => {
    if (!line || typeof line !== "object" || Array.isArray(line))
      throw new CommerceDomainError(`Line ${index + 1} is invalid.`, 400, `lineItems.${index}`);
    return line as RecordData;
  });
  const productIds = [
    ...new Set(lines.map((line) => text(line.productId)).filter((value): value is string => Boolean(value)))
  ];
  if (productIds.length !== lines.length)
    throw new CommerceDomainError("Every order line requires a Product.", 400, "lineItems.productId");
  const products = await tx.product.findMany({ where: { organizationId, id: { in: productIds } } });
  if (products.length !== productIds.length)
    throw new CommerceDomainError("One or more Products were not found.", 404, "lineItems.productId");
  const productsById = new Map(products.map((product) => [product.id, product]));
  const entries = await tx.priceBookEntry.findMany({
    where: {
      organizationId,
      productId: { in: productIds },
      ...(store.priceBookId ? { priceBookId: store.priceBookId } : {}),
      currency
    },
  ...
  entries
    ...
    .forEach((entry) => {
      if (!defaultEntryByProduct.has(entry.productId)) defaultEntryByProduct.set(entry.productId, entry);
    });
  let calculated;
  try {
    calculated = calculateInvoiceTotals(
      lines.map((line, index) => {
        ...
        if (entryId && (!entry || entry.productId !== productId))
          throw new CommerceDomainError(
            `Line ${index + 1} Price Book entry is invalid for this store.`,
            400,
            `lineItems.${index}.priceBookEntryId`
          );
        return {
          productId,
          description: text(line.description) ?? product.description ?? product.name,
          quantity: (line.quantity ?? 1) as Prisma.Decimal.Value,
          unitPrice: (line.unitPrice ?? entry?.listPrice ?? 0) as Prisma.Decimal.Value,
          discountAmount: (line.discountAmount ?? 0) as Prisma.Decimal.Value,
          taxRate: (line.taxRate ?? 0) as Prisma.Decimal.Value,
          displayOrder: Number(line.sortOrder ?? index)
        };
      })
    );
  } catch (error) {
  ...
  if (promotionCode) {
    promotion = await tx.commercePromotion.findFirst({
      where: { organizationId, storeId, code: promotionCode.toUpperCase() }
    });
    if (!promotion || !promotion.active)
      throw new CommerceDomainError("Promotion code is invalid or inactive.", 400, "promotionCode");
    const now = new Date();
    if (
      ...
      throw new CommerceDomainError("Promotion code is not currently available.", 400, "promotionCode");
    const discountedSubtotal = calculated.subtotal.minus(calculated.discountTotal);
    if (promotion.minimumOrderAmount && discountedSubtotal.lt(promotion.minimumOrderAmount))
      throw new CommerceDomainError(
        `This promotion requires an order of at least ${promotion.minimumOrderAmount.toFixed(2)}.`,
        400,
        "promotionCode"
      );
    promotionDiscount =
      promotion.type === "Percentage"
        ? money(discountedSubtotal.mul(promotion.value).div(100))
        : money(Prisma.Decimal.min(discountedSubtotal, promotion.value));
  }
  ...
  return {
    header: {
      ...
      contactId,
      currency,
      purchaseOrderNumber: text(payload.purchaseOrderNumber ?? existing?.purchaseOrderNumber),
      shippingName: text(payload.shippingName ?? existing?.shippingName) ?? account.name,
      shippingStreet: text(payload.shippingStreet ?? existing?.shippingStreet ?? account.shippingStreet),
      shippingCity: text(payload.shippingCity ?? existing?.shippingCity ?? account.shippingCity),
      shippingState: text(payload.shippingState ?? existing?.shippingState ?? account.shippingState),
      shippingPostalCode: text(
        payload.shippingPostalCode ?? existing?.shippingPostalCode ?? account.shippingPostalCode
      ),
      shippingCountry: text(payload.shippingCountry ?? existing?.shippingCountry ?? account.shippingCountry),
      notes: text(payload.notes ?? existing?.notes),
      orderDate: parseDate(payload.orderDate, "Order date") ?? existing?.orderDate ?? new Date()
    },
    calculated,
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const order = await tx.commerceOrder.create({
      data: {
        ...
        subtotal: prepared.calculated.subtotal,
        discountTotal: prepared.discountTotal,
        taxTotal: prepared.calculated.taxTotal,
        shippingTotal: prepared.shippingTotal,
        total: prepared.total,
        createdById: userId,
        lines: {
          create: prepared.calculated.lineItems.map((line) => ({
            organizationId,
            ...
            sortOrder: line.displayOrder,
            priceBookEntryId: line.productId ? (prepared.entriesByProduct.get(line.productId)?.id ?? null) : null
          }))
        },
        ...(prepared.promotion
          ? {
              promotions: {
                create: {
                  organizationId,
                  promotionId: prepared.promotion.id,
                  discountAmount: prepared.promotionDiscount
                }
              }
            }
          : {})
    ...
    const notification = await createCommerceNotification(tx, {
      ...
      userId,
      title: "Order created",
      body: `${order.orderNumber} was created as a Draft.`
    });
...
export async function updateCommerceOrder(organizationId: string, userId: string, id: string, payload: RecordData) {
  return prisma.$transaction(async (tx) => {
    ...
    await createCommerceNotification(tx, {
      organizationId,
      userId,
      title: "Order updated",
      body: `${order.orderNumber} was updated.`
    });
    return order;
  });
}
...
async function reserveInventory(
  tx: Transaction,
  organizationId: string,
  order: CommerceOrderAggregate,
  release = false
) {
  for (const line of order.lines) {
    const inventory = await tx.inventoryItem.findFirst({
      where: { organizationId, storeId: order.storeId, productId: line.productId }
    });
    if (!inventory) continue;
    const available = inventory.quantityOnHand.minus(inventory.quantityReserved);
    if (!release && available.lt(line.quantity))
      throw new CommerceDomainError(
        `Insufficient inventory for ${line.product.name}. Available: ${available.toFixed(3)}.`,
        409
      );
    await tx.inventoryItem.update({
      where: { id: inventory.id },
      data: {
        quantityReserved: release
          ? Prisma.Decimal.max(0, inventory.quantityReserved.minus(line.quantity))
          : inventory.quantityReserved.plus(line.quantity)
      }
    });
  }
}
...
export async function transitionCommerceOrder(
  organizationId: string,
  userId: string,
  id: string,
  action: string,
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (normalized === "confirm") {
      if (existing.status !== "Draft")
        throw new CommerceDomainError(`Cannot confirm an order while it is ${existing.status}.`, 409);
      if (!existing.lines.length)
        throw new CommerceDomainError("Add at least one line item before confirming the order.", 400);
      if (existing.store.status !== "Active")
        throw new CommerceDomainError("The Store must be Active before confirming an order.", 409);
      await reserveInventory(tx, organizationId, existing);
      const promotion = existing.promotions[0]
        ? await tx.commercePromotion.update({
            where: { id: existing.promotions[0].promotionId },
      ...
      const notification = await createCommerceNotification(tx, {
        ...
        body: `${order.orderNumber} was confirmed. No payment was processed by the CRM.`
      });
      return {
        order,
        promotion,
        inventoryItems: await inventoryForStore(tx, organizationId, existing.storeId),
        notifications: [notification]
      };
    }
    if (normalized === "cancel") {
      if (!(["Draft", "Confirmed"] as string[]).includes(existing.status))
        throw new CommerceDomainError(`Cannot cancel an order while it is ${existing.status}.`, 409);
      if (existing.fulfillments.some((fulfillment) => ["Shipped", "Delivered"].includes(fulfillment.status)))
        throw new CommerceDomainError("An order with shipped items cannot be cancelled.", 409);
      if (existing.status === "Confirmed") await reserveInventory(tx, organizationId, existing, true);
      const order = await tx.commerceOrder.update({
        where: { id },
        data: { status: "Cancelled", cancelledAt: new Date(), fulfillmentStatus: "Cancelled" },
        include: commerceOrderInclude
      });
      const notification = await createCommerceNotification(tx, {
        organizationId,
    ...
    if (normalized === "deliver") {
      if (existing.status !== "Fulfilled")
        throw new CommerceDomainError("Only a fully fulfilled order can be marked delivered.", 409);
      const fulfillmentId = requiredText(payload.fulfillmentId ?? existing.fulfillments[0]?.id, "Fulfillment");
      const result = await tx.commerceFulfillment.updateMany({
        where: { id: fulfillmentId, orderId: id, organizationId, status: "Shipped" },
...
async function fulfillCommerceOrder(
  ...
  if (order.status !== "Confirmed")
    throw new CommerceDomainError(`Cannot fulfill an order while it is ${order.status}.`, 409);
  const requested = Array.isArray(payload.lines)
    ? (payload.lines as RecordData[])
    : order.lines.map((line) => ({ orderLineId: line.id, quantity: line.quantity.minus(line.fulfilledQuantity) }));
  ...
  const normalized = requested.map((line, index) => {
    ...
    if (quantity.lte(0) || quantity.gt(remaining))
      throw new CommerceDomainError(
        `Fulfillment quantity for ${source.product.name} must be greater than zero and no more than ${remaining.toFixed(3)}.`,
        400
      );
    return { source, quantity };
  });
  const ordinal = order.fulfillments.length + 1;
  const fulfillment = await tx.commerceFulfillment.create({
    data: {
      organizationId,
      orderId: order.id,
      fulfillmentNumber: `${order.orderNumber}-F${String(ordinal).padStart(2, "0")}`,
      status: text(payload.status) === "Packed" ? "Packed" : "Shipped",
      carrier: text(payload.carrier),
      trackingNumber: text(payload.trackingNumber),
      notes: text(payload.notes),
      createdById: userId,
      packedAt: new Date(),
      shippedAt: text(payload.status) === "Packed" ? null : new Date(),
      lines: {
  ...
  for (const { source, quantity } of normalized) {
    await tx.commerceOrderLine.update({
      where: { id: source.id },
      data: { fulfilledQuantity: { increment: quantity } }
    });
    const inventory = await tx.inventoryItem.findFirst({
      where: { organizationId, storeId: order.storeId, productId: source.productId }
    });
    if (inventory)
      await tx.inventoryItem.update({
        where: { id: inventory.id },
        data: {
          quantityOnHand: Prisma.Decimal.max(0, inventory.quantityOnHand.minus(quantity)),
          quantityReserved: Prisma.Decimal.max(0, inventory.quantityReserved.minus(quantity))
        }
      });
  }
  ...
  const updated = await tx.commerceOrder.update({
    where: { id: order.id },
    data: {
      status: complete ? "Fulfilled" : "Confirmed",
      fulfillmentStatus: complete ? "Fulfilled" : "Partially Fulfilled",
  ...
  const notification = await createCommerceNotification(tx, {
    organizationId,
    userId,
    title: complete ? "Order fulfilled" : "Order partially fulfilled",
    body: `${updated.orderNumber} fulfillment ${fulfillment.fulfillmentNumber} was recorded.`
  });
  return {
    order: updated,
    fulfillment,
    inventoryItems: await inventoryForStore(tx, organizationId, order.storeId),
    notifications: [notification]
  };
}
...
export async function upsertInventory(organizationId: string, storeId: string, payload: RecordData) {
  ...
  if (!(await prisma.product.findFirst({ where: { id: productId, organizationId }, select: { id: true } })))
    throw new CommerceDomainError("Product not found.", 404, "productId");
  ...
  if (quantityOnHand.lt(0) || reorderPoint.lt(0))
    throw new CommerceDomainError("Inventory quantities cannot be negative.", 400);
  const existing = await prisma.inventoryItem.findUnique({
    where: { organizationId_storeId_productId: { organizationId, storeId, productId } }
  });
  if (existing && quantityOnHand.lt(existing.quantityReserved))
    throw new CommerceDomainError(
      `Quantity on hand cannot be below the reserved quantity (${existing.quantityReserved.toFixed(3)}).`,
      409
    );
  return prisma.inventoryItem.upsert({
    where: { organizationId_storeId_productId: { organizationId, storeId, productId } },
    update: { quantityOnHand, reorderPoint },
    create: { organizationId, storeId, productId, quantityOnHand, reorderPoint },
...
export async function createPromotion(organizationId: string, userId: string, storeId: string, payload: RecordData) {
  ...
  if (!(PROMOTION_TYPES as readonly string[]).includes(type))
    throw new CommerceDomainError("Choose a valid promotion type.", 400, "type");
  const value = decimal(payload.value, "Promotion value");
  if (value.lte(0) || (type === "Percentage" && value.gt(100)))
    throw new CommerceDomainError(
      type === "Percentage"
        ? "Percentage promotions must be greater than 0 and no more than 100."
  ...
  if (startsAt && endsAt && endsAt < startsAt)
    throw new CommerceDomainError("Promotion end date cannot precede its start date.", 400, "endsAt");
  const minimumOrderAmount = payload.minimumOrderAmount
    ? decimal(payload.minimumOrderAmount, "Minimum order amount")
    : null;
  ...
  if (maxRedemptions !== null && (!Number.isInteger(maxRedemptions) || maxRedemptions <= 0))
    throw new CommerceDomainError("Maximum redemptions must be a positive whole number.", 400, "maxRedemptions");
  if (await prisma.commercePromotion.findFirst({ where: { organizationId, storeId, code }, select: { id: true } }))
    throw new CommerceDomainError("That promotion code already exists for this store.", 409, "code");
  return prisma.commercePromotion.create({
    data: {
      organizationId,
      storeId,
      name,
...
export function commerceErrorResponse(error: unknown) {
  return error instanceof CommerceDomainError
    ? { error: error.message, field: error.field, status: error.status }
    : null;
}
