...
export const MARKETING_FORM_FIELDS = ["firstName", "lastName", "email", "company", "phone", "title", "message"] as const;
...
export function marketingPageErrorResponse(error: unknown) {
  ...
  if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") return { error: "A landing page with that name or URL slug already exists.", status: 409 as const };
  return null;
}
...
export function landingPageFields(value: unknown) {
  ...
  const allowed = requested.filter((field): field is typeof MARKETING_FORM_FIELDS[number] => (MARKETING_FORM_FIELDS as readonly string[]).includes(field));
...
export async function requireLandingPage(organizationId: string, id: string) {
  const page = await prisma.marketingLandingPage.findFirst({ where: { id, organizationId }, include: marketingLandingPageInclude });
  if (!page) throw new MarketingPageError("Landing page not found.", 404);
  return page;
}

export async function validateLandingPageReferences(organizationId: string, ownerId: string, campaignId: string | null) {
  const member = await prisma.organizationMembership.findFirst({ where: { organizationId, userId: ownerId, status: "ACTIVE", user: { status: "ACTIVE" } }, select: { userId: true } });
...
export async function createMarketingPageNotification(client: NotificationClient, values: { organizationId: string; userId: string; title: string; body: string; href?: string }) {
  return client.notification.create({
    data: { organizationId: values.organizationId, userId: values.userId, title: values.title, body: values.body, href: values.href ?? "/lightning/app/marketing", category: "Marketing" }
  });
}
